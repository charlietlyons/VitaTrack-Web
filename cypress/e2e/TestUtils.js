...
export function mockAddIntake() {
  cy.intercept("POST", "/intake", (req) => {
    const foodOption = mockFoodOptions.find(
      (option) => option._id === req.body.foodId
    );
    mockIntakes.push({
      _id: "someId",
      userId: "someUserId",
...
export function mockGetIntakes() {
  mockIntakes.forEach((intake) => {
    const foodData = mockFoodOptions.find(
      (option) => option._id === intake.foodId
    );
    if (foodData) {
...
export function mockGetFirstIntakeById() {
  cy.intercept("GET", "/intake/6b99dc4c-2299-4ad2-8299-08cd7e3de025", {
    statusCode: 200,
    body: mockIntakes.find(
      (intake) => intake._id === "6b99dc4c-2299-4ad2-8299-08cd7e3de025"
    ),
  });
}

export function mockGetSecondIntakeById() {
  cy.intercept("GET", "/intake/someId", {
    statusCode: 200,
    body: mockIntakes[1],
  });
}
